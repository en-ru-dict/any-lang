...
if (!window.g_AnyLang) {
	window.g_AnyLang = (() => {
		...
		let config = {
			mode: 'dev',               // 'prod' (fast regex) or 'dev' (slow accurate)
		...
		// MODE 2: 'dev' - Accurate, Multi-Pass split/join Replacement (Slower, but reliable count)
		function replace_slow_accurate(html, dict) {
			...
			log(`Slow (Dev) Mode: Replaced ${config.replace} strings.`);
			return html;
		}
		...
		function replace_html(targetLang) {
			...
			if (config.mode === 'dev') html =  replace_slow_accurate(html, dict);
			else {config.replace=1;html = html.replace(/\[·([^·]+)·\]/g, (v,k) => dict.get(k) || 'no tr?['+k+']');}
